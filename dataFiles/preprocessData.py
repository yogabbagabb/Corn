...
def load_yield_data():
    ...
    # data = data.round(8)
    # g = (lambda x: int(x) if (isinstance(x,float) and x.is_integer()) else x)
    # data = data.applymap(g)
    # data = data.astype(object)
    # data.to_csv("no_irr_Python", index=False)
...
if __name__ == "__main__":
    ...
    data.to_csv("processed_Corn_model_data.csv", index=False)
